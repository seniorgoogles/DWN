...
def main(args):
    ...
    elif thermometer_type == "adaptive_dense_thermometer": 
        print("Using adaptive dense thermometer")
        thermometer = dwn.AdaptiveThermometer(thermometer_bits, method="density").fit(x_train)
    elif thermometer_type == "adaptive_variance_thermometer":
        print("Using adaptive variance thermometer")
        thermometer = dwn.AdaptiveThermometer(thermometer_bits, method="variance").fit(x_train)
    else:
    ...
    best_acc = train_and_evaluate(model, optimizer, scheduler, x_train, y_train, x_test, y_test, epochs=epochs, batch_size=batch_size, device=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--luts-num', type=int, default=50, help='Number of LUTs')
    parser.add_argument('--luts-inp-num', type=int, default=6, help='Number of inputs per LUT')
    parser.add_argument('--thermometer', '-t', type=str, default="uniform_thermometer", help='Thermometer type, thermometer or distributive_thermometer')
    ...
    # Print command 
    str_cmd = " ".join([f"{k}={v}" for k, v in vars(args).items()])
    print(f"Running with command: {str_cmd}")
