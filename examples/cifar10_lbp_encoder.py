"""
CIFAR10 Image Classification using DWN (Deep Weight-sharing Network)
This example demonstrates using EncoderLayer with thermometer encoding for CIFAR10 classification.
...
def plot_thresholds_comparison(
    initial_thresholds,
    final_thresholds,
    train_data,
    features_to_plot=range(16),
    input_min=-1.0,
    input_max=0.999999999999999,
):
    ...
    for feature_idx in features_to_plot:
        ...
        fig.suptitle(
            f"Feature {feature_idx}: Data Distribution and Thresholds Analysis",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax1.hist(
            feature_data, bins=100, color="lightblue", edgecolor="black", alpha=0.7
        )
        ...
        ax2.plot(
            initial_thresholds[feature_idx],
            "-",
            label="Initial",
            color="blue",
            linewidth=2,
        ...
        ax2.axhline(
            ...
            linestyle="--",
            alpha=0.3,
            label=f"Range: [{input_min:.1f}, {input_max:.2f}]",
        )
        ...
        initial_with_bounds = np.concatenate(
            [[input_min], initial_thresholds[feature_idx], [input_max]]
        )
        initial_spacings = np.diff(initial_with_bounds)
        final_with_bounds = np.concatenate(
            [[input_min], final_thresholds[feature_idx], [input_max]]
        )
        ...
        plt.savefig(f"cifar10_feature{feature_idx}.png", dpi=150, bbox_inches="tight")
        plt.close()
        print(f"Saved plot: cifar10_feature{feature_idx}.png")
...
def train_and_evaluate(
    ...
    """Train model and evaluate after each epoch with optional mixed precision"""
    ...
    if use_amp and AMP_AVAILABLE:
        ...
        print("Mixed Precision Training: DISABLED (not available)")
    print("=" * 70)
    ...
    for epoch in range(epochs):
        ...
        print(f"\nEpoch {epoch + 1}/{epochs} - Processing {num_batches} batches (batch_size={batch_size})...")
        ...
        for batch_idx, i in enumerate(range(0, n_samples, batch_size)):
            ...
            if (batch_idx + 1) % 100 == 0 or batch_idx == 0:
                avg_loss = running_loss / min(batch_idx + 1, 100)
                current_acc = correct_train / total_train
                print(f"  Batch {batch_idx + 1}/{num_batches} | Loss: {avg_loss:.4f} | Acc: {current_acc:.4f}")
                running_loss = 0.0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="CIFAR10 Image Classification with DWN EncoderLayer"
    )
    parser.add_argument(
        "--estimator",
        type=str,
        default="ste",
        choices=["ste", "finite_difference", "glt"],
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-2, help="Learning rate (default: 1e-3)"
    )
    parser.add_argument(
        "--hidden-size", type=int, default=16000, help="Hidden layer size (default: 10)"
    )
    parser.add_argument(
        "--thermometer-bits",
        type=int,
        default=30,
        help="Number of thermometer bits per feature (default: 10)",
    )
    parser.add_argument(
        "--encoding-type",
        type=str,
        default="thermometer",
        choices=["thermometer", "lbp", "lbp+thermometer", "lbp_distributive", "lbp_double_thermo"],
        help="Encoding type: thermometer, lbp, lbp+thermometer, lbp_distributive, or lbp_double_thermo (default: thermometer)",
    )
    parser.add_argument(
        "--thermo-type",
        type=str,
        default="distributive",
        choices=["uniform", "gaussian", "distributive"],
        help="Thermometer encoding type: uniform, gaussian, or distributive (default: distributive)",
    )
    parser.add_argument(
        "--data-dir",
        type=str,
        default="./data",
        help="Directory to store CIFAR10 data (default: ./data)",
    )
    parser.add_argument(
        "--mixed-precision",
        action="store_true",
        default=True,
        help="Enable mixed precision (FP16) training for faster training (default: True)",
    )
    parser.add_argument(
        "--compile",
        action="store_true",
        default=False,
        help="Use torch.compile for faster execution (requires PyTorch 2.0+, default: False)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of random samples to use for fitting thresholds (default: use all)",
    )
    args = parser.parse_args()

    print("=" * 70)
    print("CIFAR10 Image Classification with DWN EncoderLayer")
    print("=" * 70)
    print(f"\nConfiguration:")
    print(f"  - Encoding type: {args.encoding_type}")
    print(f"  - Thermometer type: {args.thermo_type}")
    print(f"  - Estimator: {args.estimator}")
    print(
        f"  - Thresholds: {'Fixed (non-trainable)' if args.fixed_thresholds else 'Trainable'}"
    )
    ...
    print(f"\nOptimizations:")
    print(f"  - Mixed precision: {args.mixed_precision and AMP_AVAILABLE}")
    ...
    # Convert to numpy arrays and convert RGB to grayscale
    # RGB to grayscale conversion using luminosity method: 0.299*R + 0.587*G + 0.114*B
    x_train_rgb = train_dataset.data.astype(np.float32)  # Shape: (50000, 32, 32, 3)
    x_train_gray = (
        0.299 * x_train_rgb[:, :, :, 0]
        + 0.587 * x_train_rgb[:, :, :, 1]
        + 0.114 * x_train_rgb[:, :, :, 2]
    )
    x_train = x_train_gray.reshape(len(train_dataset.data), -1)  # Shape: (50000, 1024)
    y_train = np.array(train_dataset.targets, dtype=np.int64)

    x_test_rgb = test_dataset.data.astype(np.float32)  # Shape: (10000, 32, 32, 3)
    x_test_gray = (
        0.299 * x_test_rgb[:, :, :, 0]
        + 0.587 * x_test_rgb[:, :, :, 1]
        + 0.114 * x_test_rgb[:, :, :, 2]
    )
    x_test = x_test_gray.reshape(len(test_dataset.data), -1)  # Shape: (10000, 1024)
    ...
    print(
        f"  - Features shape: {x_train.shape} (flattened from 32x32 grayscale images)"
    )
    ...
    plot_dataset(x_train, filename="before_cifar10_train_dataset.png")
    plot_dataset(x_test, filename="before_cifar10_test_dataset.png")

    # Only clip/normalize for thermometer encoding (not LBP-based encodings)
    if args.encoding_type == 'thermometer':
        ...
        plot_dataset(x_train, filename="after_cifar10_train_dataset.png")
        plot_dataset(x_test, filename="after_cifar10_test_dataset.png")

        print(f"  - Min value: {x_train.min().item():.4f}")
        print(f"  - Max value: {x_train.max().item():.4f}")
    else:
        print(f"\nSkipping normalization for {args.encoding_type} encoding (LBP works on raw pixel values)")
        print(f"  - Min value: {x_train.min().item():.4f}")
        print(f"  - Max value: {x_train.max().item():.4f}")

    print(
        f"\nUsing EncoderLayer with DistributiveThermometer ({args.thermometer_bits} bits)..."
    )
    print(f"Original features shape: {x_train.shape}")
    print(
        f"  - Each feature will be converted to {args.thermometer_bits} binary values"
    )
    ...
    # Calculate input size to LUTLayer based on encoding type
    if args.encoding_type == 'lbp':
        # LBP only: no thermometer dimension
        lut_input_size = num_features
    elif args.encoding_type == 'lbp+thermometer':
        # LBP+Thermometer: 8 LBP bits + num_bits thermometer per pixel
        lut_input_size = num_features * (8 + args.thermometer_bits)
    elif args.encoding_type == 'lbp_distributive':
        # LBP then thermometer on LBP values
        lut_input_size = num_features * args.thermometer_bits
    elif args.encoding_type == 'lbp_double_thermo':
        # LBP → Thermo1 → sum → Thermo2 (output is num_bits2 = thermometer_bits)
        lut_input_size = num_features * args.thermometer_bits
    else:
        # Thermometer: has thermometer dimension
        lut_input_size = num_features * args.thermometer_bits

    # Sample data for fitting thresholds if --fit-samples is specified
    if args.fit_samples is not None and args.fit_samples < len(x_train):
        print(f"\nUsing {args.fit_samples} random samples for threshold fitting...")
        fit_idx = torch.randperm(len(x_train))[:args.fit_samples]
        fit_data = x_train[fit_idx]
    else:
        fit_data = x_train
    ...
    model = nn.Sequential(
        EncoderLayer(
            num_features,
            args.thermometer_bits,
            input_dataset=fit_data,
            estimator_type=args.estimator,
            encoding_type=args.encoding_type,
            image_shape=(32, 32),  # CIFAR10 grayscale images are 32x32
        ...
        dwn.GroupSum(k=num_classes, tau=1 / 0.03),
    )
    ...
    # Set threshold learning based on argument (only for thermometer-based encodings)
    if model[0].thresholds is not None:
        ...
        print("\nNo thresholds (LBP encoding only)")
    ...
    # Save initial thresholds for comparison (sorted) - only for thermometer-based encodings
    if model[0].thresholds is not None:
        print("\nSaving initial thresholds for plotting...")
        # Handle both 1D (LbpThermometer) and 2D (regular Thermometer) thresholds
        if model[0].thresholds.ndim == 1:
            # 1D thresholds (LbpThermometer) - no sorting needed along dim 1
            initial_thresholds = torch.sort(model[0].thresholds.data)[0].clone().unsqueeze(0)
        else:
            # 2D thresholds (regular Thermometer)
    ...
    # Plot threshold comparisons for first and last features (sorted) - only for thermometer-based encodings
    if initial_thresholds is not None:
        print("\nGenerating threshold comparison plots for first and last features...")
        # Handle both 1D and 2D thresholds
        if model[0].thresholds.ndim == 1:
            final_thresholds = torch.sort(model[0].thresholds.data)[0].unsqueeze(0)
        else:
            final_thresholds = torch.sort(model[0].thresholds.data, dim=1)[0]
        num_features = x_train.size(1)
        plot_thresholds_comparison(
            initial_thresholds, final_thresholds, x_train, features_to_plot=[0, num_features - 1]
        )
        print("All plots saved!")
    else:
        print("\nSkipping threshold plots (LBP encoding only)")
    ...
    if hasattr(lut_layer, "mapping"):
        ...
        print(
            f"  - Average connections per input: {input_connections[input_connections > 0].mean():.2f}"
        )
        print(
            f"  - Max connections to single input: {input_connections.max().item():.0f}"
        )
        print(
            f"  - Min connections (non-zero): {input_connections[input_connections > 0].min().item():.0f}"
        )
    ...
    print(
        f"  - Entries < {threshold}: {sparse_entries} / {total_entries} ({100 * sparse_entries / total_entries:.1f}%)"
    )
    ...
    report_filename = "cifar10_lut_report.md"
    with open(report_filename, "w") as f:
        f.write("# CIFAR10 LUT Layer Report\n\n")
        ...
        if hasattr(lut_layer, "mapping"):
            ...
            for i in range(lut_layer.output_size):
                ...
                # Input mapping
                inputs = (
                    discrete_mapping[i].numpy()
                    if hasattr(discrete_mapping[i], "numpy")
                    else discrete_mapping[i]
                )
                ...
                f.write(
                    f"- Ones: {ones_count} / {len(tt_binary)} ({100 * ones_count / len(tt_binary):.1f}%)\n"
                )
                f.write(
                    f"- Zeros: {zeros_count} / {len(tt_binary)} ({100 * zeros_count / len(tt_binary):.1f}%)\n\n"
                )
        ...
        if hasattr(lut_layer, "mapping"):
            ...
            f.write(
                f"- Overall binary distribution: {overall_ones} ones / {overall_total} total ({100 * overall_ones / overall_total:.1f}% ones)\n"
            )
